 ...
 */
#define OSPI_B_PRV_UNIT_MASK(p_ext_cfg) (OSPI_B_PRV_UNIT_CHANNELS_MASK << (((p_ext_cfg)->ospi_b_unit) * OSPI_B_PRV_UNIT_CHANNELS_SHIFT))
 ...
 */
#define OSPI_B_PRV_CH_MASK(p_ext_cfg) ((1U << ((p_ext_cfg)->channel)) << (((p_ext_cfg)->ospi_b_unit) * OSPI_B_PRV_UNIT_CHANNELS_SHIFT))

/**
 * Gets the extended configuration struct for this instance.
 * @param p_ctrl Pointer to the instance control block.
 */
#define OSPI_B_PRV_EXTENDED_CFG(p_ctrl) ((ospi_b_extended_cfg_t *)((ospi_b_instance_ctrl_t *)(p_ctrl))->p_cfg->p_extend)
...
#define OSPI_B_PRV_ADDRESS_REPLACE_ENABLE_BITS (OSPI_B_PRV_ADDRESS_REPLACE_VALUE << R_XSPI0_CMCFGCS_CMCFG0_ADDRPEN_Pos)
    ...
    spi_flash_direct_transfer_t direct_command =
        {
            ...
            .command_length = (uint8_t)p_cmd_set->command_bytes,
            .address_length = (uint8_t)(p_cmd_set->status_needs_address ? OSPI_B_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->status_address_bytes) : 0U),
...
static fsp_err_t r_ospi_b_automatic_calibration_seq(ospi_b_instance_ctrl_t *p_instance_ctrl)
{
    ...
    p_reg->CCCTLCS[channel].CCCTL0 =
        ...
        (OSPI_B_PRV_AUTOCALIBRATION_FRAME_INTERVAL << R_XSPI0_CCCTLCS_CCCTL0_CAITV_Pos) |
        (OSPI_B_PRV_AUTOCALIBRATION_SHIFT_DS_END_VALUE << R_XSPI0_CCCTLCS_CCCTL0_CASFTEND_Pos);
                                     ...
                                     spi_flash_direct_transfer_dir_t direction)
{
    ...
    cdtbuf0 |= (1 == p_transfer->command_length) ? ((p_transfer->command & OSPI_B_PRV_CDTBUF_CMD_1B_VALUE_MASK) << OSPI_B_PRV_CDTBUF_CMD_UPPER_OFFSET) : ((p_transfer->command & OSPI_B_PRV_CDTBUF_CMD_2B_VALUE_MASK) << OSPI_B_PRV_CDTBUF_CMD_OFFSET);
...
static void r_ospi_b_xip(ospi_b_instance_ctrl_t *p_instance_ctrl, bool is_entering)
{
    ...
    {
        dummy_read_address = ((OSPI_B_DEVICE_NUMBER_0 == p_instance_ctrl->channel) ? BSP_OSPI0_CS0_START_ADDRESS : BSP_OSPI0_CS1_START_ADDRESS);
    }
    else if (1U == p_instance_ctrl->ospi_b_unit)
    {
        dummy_read_address = ((OSPI_B_DEVICE_NUMBER_0 == p_instance_ctrl->channel) ? BSP_OSPI1_CS0_START_ADDRESS : BSP_OSPI1_CS1_START_ADDRESS);
    }
    else
    {
        /* Not a valid unit. */
