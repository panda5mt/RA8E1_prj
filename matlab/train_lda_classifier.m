...
function save_lda_parameters(W, b, lda_model, output_dir, class_names)
    ...
    try
        ...
        fprintf('  -> src/hlac_lda_model.c にコピーしてください\n');
    catch ME
        fprintf('WARNING: RA8E1用 hlac_lda_model.c 生成に失敗: %s\n', ME.message);
    end
...
function generate_ra8e1_hlac_lda_model_c(W, b, lda_model, output_dir)
% Generate RA8E1 firmware-ready model file compatible with:
%   src/hlac_lda_model.h / src/hlac_lda_model.c
%
% This emits:
%   - g_hlac_lda_num_classes
%   - g_hlac_lda_W[25][10]
%   - g_hlac_lda_b[10]
%   - g_hlac_feature_mean[25]
%   - g_hlac_feature_std[25]
%
% NOTE: W,b are trained on standardized features (z-scored).
% The firmware applies (x-mean)/std before scoring, so we also export mean/std.

    if nargin < 4
        error('generate_ra8e1_hlac_lda_model_c: invalid args');
    end

    [D, K] = size(W);
    if D ~= 25
        fprintf('WARNING: HLAC feature dim is %d (expected 25). Exporting anyway.\n', D);
    end

    mu = zeros(1, D);
    sigma = ones(1, D);
    if isstruct(lda_model)
        if isfield(lda_model, 'feature_mean')
            mu = lda_model.feature_mean;
        end
        if isfield(lda_model, 'feature_std')
            sigma = lda_model.feature_std;
        end
    elseif ~isempty(lda_model)
        try
            if isprop(lda_model, 'feature_mean')
                mu = lda_model.feature_mean;
            end
            if isprop(lda_model, 'feature_std')
                sigma = lda_model.feature_std;
            end
        catch
            % ignore
    ...
    % RA8E1 side uses MAX_CLASSES=10 fixed arrays; pad remaining columns with 0.
    MAXC = 10;
    if K > MAXC
        error('num_classes=%d exceeds MAX_CLASSES=%d on firmware side', K, MAXC);
    end

    out_c = fullfile(output_dir, 'hlac_lda_model.c');
    fid = fopen(out_c, 'w');
    if fid < 0
        error('Failed to open %s', out_c);
    end

    cleanup = onCleanup(@() fclose(fid));

    fprintf(fid, '/* Auto-generated by MATLAB: train_lda_classifier.m */\n');
    fprintf(fid, '/* Copy this file to: src/hlac_lda_model.c */\n');
    fprintf(fid, '/* Date: %s */\n\n', datestr(now));
    fprintf(fid, '#include "hlac_lda_model.h"\n\n');

    fprintf(fid, 'const uint32_t g_hlac_lda_num_classes = %dU;\n\n', K);

    % W: [25 x 10]
    fprintf(fid, 'const float g_hlac_lda_W[HLAC_FEATURE_DIM][HLAC_MAX_CLASSES] = {\n');
    for i = 1:25
        fprintf(fid, '    {');
        for c = 1:MAXC
            if (i <= D) && (c <= K)
    ...
    % b: [10]
    fprintf(fid, 'const float g_hlac_lda_b[HLAC_MAX_CLASSES] = {\n');
    for c = 1:MAXC
        if c <= K
            v = b(c);
        else
    ...
    % mean/std: [25]
    fprintf(fid, 'const float g_hlac_feature_mean[HLAC_FEATURE_DIM] = {\n');
    for i = 1:25
        if i <= numel(mu)
            v = mu(i);
        else
    ...
    fprintf(fid, 'const float g_hlac_feature_std[HLAC_FEATURE_DIM] = {\n');
    for i = 1:25
        if i <= numel(sigma)
            v = sigma(i);
        else
